...
- 数字+助数詞は `<NUM>` 正規化により汎化される: `<NUM>週間/<NUM>しゅうかん` のようにかな読みを保持した形で集約。セグメンタが数字+かなの複合セグメントも生成するため、「90ぎょう」→「90行」のような変換も可能
...
評価スコアが変動した場合（コーパス追加、corpus-stats 更新、辞書変更等）、README.md の「評価スコア履歴」テーブルに新しいエントリを追加すること。
